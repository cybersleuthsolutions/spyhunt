...
group.add_argument('-sv',
                   '--save',
                   action='store',
                   help="save output to file",
                   metavar="filename.txt")

group.add_argument('-wl',
                   '--wordlist',
                   action='store',
...
passiverecon_group.add_argument('-s',
                                type=str,
                                help='scan for subdomains',
                                metavar='domain.com')

passiverecon_group.add_argument('-t',
                                '--tech',
                                type=str,
                                help='find technologies',
                                metavar='domain.com')

passiverecon_group.add_argument('-d',
                                '--dns',
                                type=str,
...
vuln_group.add_argument('-b',
                        '--brokenlinks',
                        type=str,
                        help='search for broken links',
                        metavar='domains.txt')

crawlers_group.add_argument('-pspider',
                            '--paramspider',
                            type=str,
                            help='extract parameters from a domain',
                            metavar='domain.com')

crawlers_group.add_argument('-w',
                            '--waybackurls',
                            type=str,
...
crawlers_group.add_argument('-wc',
                            '--webcrawler',
                            type=str,
...
passiverecon_group.add_argument('-na',
                                '--networkanalyzer',
                                type=str,
                                help='net analyzer',
                                metavar='https://domain.com')
...
vuln_group.add_argument('-ph',
                        '--pathhunt',
                        type=str,
                        help='check for directory traversal',
                        metavar='domain.txt')

vuln_group.add_argument('-co',
                        '--corsmisconfig',
                        type=str,
                        help='cors misconfiguration',
                        metavar='domains.txt')

vuln_group.add_argument('-hh',
                        '--hostheaderinjection',
                        type=str,
...
vuln_group.add_argument('-smu',
                        '--smuggler',
                        type=str,
                        help='enumerate domains',
                        metavar='domain.com')

passiverecon_group.add_argument('-ips',
                                '--ipaddresses',
                                type=str,
...
fuzzing_group.add_argument('-nft',
                           '--not_found',
                           type=str,
                           help='check for 404 status code',
                           metavar='domains.txt')

portscanning_group.add_argument('-n',
                                '--nmap',
                                type=str,
                                help='Scan a target with nmap',
                                metavar='domain.com or IP')

fuzzing_group.add_argument('-api',
                           '--api_fuzzer',
                           type=str,
                           help='Look for API endpoints',
                           metavar='domain.com')

passiverecon_group.add_argument('-sho',
                                '--shodan',
                                type=str,
                                help='Recon with shodan',
                                metavar='domain.com')

vuln_group.add_argument('-fp',
                        '--forbiddenpass',
                        type=str,
                        help='Bypass 403 forbidden',
                        metavar='domain.com')

fuzzing_group.add_argument('-db',
                           '--directorybrute',
                           type=str,
...
portscanning_group.add_argument('-ps',
                                '--ports',
                                type=str,
                                help='Port numbers to scan',
                                metavar='80,443,8443')

portscanning_group.add_argument('-pai',
                                '--print_all_ips',
                                type=str,
                                help='Print all ips',
                                metavar='IP/24')

vuln_group.add_argument('-xss',
                        '--xss_scan',
                        type=str,
                        help='scan for XSS vulnerabilities',
                        metavar='https://example.com/page?param=value')

vuln_group.add_argument('-sqli',
                        '--sqli_scan',
                        type=str,
                        help='scan for SQLi vulnerabilities',
                        metavar='https://example.com/page?param=value')

passiverecon_group.add_argument('-shodan',
                                '--shodan_api',
                                type=str,
                                help='shodan api key',
                                metavar='KEY')
...
crawlers_group.add_argument('-javascript',
                            '--javascript_scan',
                            type=str,
                            help='scan for sensitive info in javascript files',
                            metavar='domain.com')

crawlers_group.add_argument('-dp',
                            '--depth',
                            type=str,
                            help='depth of the crawl',
                            metavar='10')

crawlers_group.add_argument('-je',
                            '--javascript_endpoints',
                            type=str,
                            help='extract javascript endpoints',
                            metavar='file.txt')

crawlers_group.add_argument('-hibp',
                            '--haveibeenpwned',
                            type=str,
                            help='check if the password has been pwned',
                            metavar='password')

fuzzing_group.add_argument('-pm',
                           '--param_miner',
                           type=str,
                           help='param miner',
                           metavar='domain.com')

fuzzing_group.add_argument('-ch',
                           '--custom_headers',
                           type=str,
                           help='custom headers',
                           metavar='domain.com')

vuln_group.add_argument('-or',
                        '--openredirect',
                        type=str,
                        help='open redirect',
                        metavar='domain.com')

fuzzing_group.add_argument('-asn',
                           '--automoussystemnumber',
                           type=str,
                           help='asn',
                           metavar='AS55555')

vuln_group.add_argument('-st',
                        '--subdomaintakeover',
                        type=str,
                        help='subdomain takeover',
                        metavar='subdomains.txt')

fuzzing_group.add_argument('-ar',
                           '--autorecon',
                           type=str,
                           help='auto recon',
                           metavar='domain.com')

vuln_group.add_argument('-jwt',
                        '--jwt_scan',
                        type=str,
                        help='analyze JWT token for vulnerabilities',
                        metavar='token')

vuln_group.add_argument('-jwt-modify',
                        '--jwt_modify',
                        type=str,
                        help='modify JWT token',
                        metavar='token')

vuln_group.add_argument('-heapds',
                        '--heapdump_file',
                        type=str,
                        help='file for heapdump scan',
                        metavar='heapdump.txt')

vuln_group.add_argument('-heapts',
                        '--heapdump_target',
                        type=str,
...
nuclei_group.add_argument('-nl',
                          '--nuclei_lfi',
                          action='store_true',
                          help="Find Local File Inclusion with nuclei")

passiverecon_group.add_argument('-gs',
                                '--google',
                                action='store_true',
                                help='Google Search')
...
update_group.add_argument('-u',
                          '--update',
                          action='store_true',
                          help='Update the script')

parser.add_argument('--shodan-api',
                    help='Shodan API key for subdomain enumeration')

parser.add_argument('--proxy',
                    help='Use a proxy (e.g., http://proxy.com:8080)')
...
async def update_script():
    try:
        ...
        try:
            ...
            print(
                f"{Fore.GREEN}Backup created at: {backup_path}{Style.RESET_ALL}"
            )
        ...
        try:
            ...
            if current == remote:
                print(
                    f"{Fore.GREEN}SpyHunt is already up to date!{Style.RESET_ALL}"
                )
                return True

        except subprocess.CalledProcessError as e:
            print(
                f"{Fore.RED}Failed to check for updates: {str(e)}{Style.RESET_ALL}"
            )
            return False
        ...
        try:
            ...
            if "Already up to date" in result.stdout:
                ...
                if os.path.exists(requirements_path):
                    print(
                        f"{Fore.CYAN}Updating dependencies...{Style.RESET_ALL}"
                    )
                    subprocess.run([
                        "pip", "install", "-r", requirements_path, "--upgrade"
                                   ...
                                   check=True)
                    print(
                        f"{Fore.GREEN}Dependencies updated!{Style.RESET_ALL}")
        ...
        print(
            f"{Fore.RED}An unexpected error occurred: {str(e)}{Style.RESET_ALL}"
        )
        return False
    ...
    if args.save:
        ...
        if args.shodan_api:
            ...
            try:
                ...
                print(Fore.GREEN +
                      f"Added {len(shodan_subdomains)} subdomains from Shodan")
        ...
        if args.shodan_api:
            ...
            try:
                ...
                for subdomain in sorted(shodan_subdomains):
                    print(subdomain)
                print(Fore.GREEN +
                      f"Found {len(shodan_subdomains)} subdomains from Shodan")
            except shodan.APIError as e:
                print(Fore.RED + f"Error querying Shodan: {e}")
...
if args.enumeratedomain:
    try:
        server = []
        r = requests.get(f"{args.enumeratedomain}",
                         verify=False,
                         headers=header)
    ...
    with open(f"{args.faviconmulti}") as f:
        ...
        try:
            for domainlist in domains:
                response = requests.get(f'{domainlist}/favicon.ico',
                                        verify=False,
                                        timeout=60,
                                        headers=header)
...
if args.corsmisconfig:
    ...
    def check_cors(domainlist):
        try:
            ...
            resp = session.get(domainlist,
                               verify=False,
                               headers=header,
                               timeout=(5, 10))

            allow_origin = resp.headers.get("Access-Control-Allow-Origin", "")
            allowed_methods = resp.headers.get(
            ...
            print(
                f"{Fore.LIGHTBLACK_EX}Error processing {domainlist}: {e}{Fore.RESET}"
            )
...
if args.hostheaderinjection:
    ...
    def setup_proxies(proxy=None, proxy_file=None):
        ...
        if proxy_file:
            try:
                ...
                print(
                    f"{Fore.RED}Error loading proxy file: {str(e)}{Style.RESET_ALL}"
                )
        return proxies
    ...
    def check_host_header_injection(domainlist):
        ...
        try:
            ...
            normal_resp = session.get(domainlist,
                                      verify=False,
                                      timeout=5,
            ...
            for header_name, header_value in headers.items():
                try:
                    ...
                    if resp.text != normal_content:
                        if 'evil.com' in resp.text.lower():
                            ...
                            if "Evil.Com" in title:
                                ...
                                print(
                                    f"{Fore.YELLOW}Title: {Fore.GREEN}{title}")
                                return
                            else:
                                pass
                    ...
                    print(
                        f"{Fore.RED}Proxy connection timeout{Style.RESET_ALL}")
                    continue
...
if args.securityheaders:
    ...
    for item, key in resp.headers.items():
        for sec_headers in security_headers:
            if sec_headers == item or lower == item or capital == item:
                ...
                [
                    no_dup_found.append(x) for x in found_hd
                    if x not in no_dup_found
                ]
    ...
    print("\n")
    print(
        f"{Fore.GREEN} Found Security Headers: {Fore.YELLOW} {len(no_dup_found)}\n"
    )
    ...
    print(
        f"{Fore.RED} Found Missing headers: {Fore.YELLOW} {len(no_headers)}\n")
...
if args.j:
    ...
    async def fetch(session, url):
        try:
            ...
            print(
                f"{Fore.RED}Unexpected error fetching {url}: {e}{Style.RESET_ALL}"
            )
        return None
    ...
    def is_same_domain(url, domain):
        try:
            ...
            print(
                f"{Fore.RED}Error comparing domains for {url}: {e}{Style.RESET_ALL}"
            )
            return False
        ...
        if html:
            try:
                ...
                for script in soup.find_all('script', src=True):
                    ...
                    if is_valid_url(script_url) and is_same_domain(
                            script_url, domain):
                        js_links.add(script_url)

                for script in soup.find_all('script'):
                    if script.string:
                        js_urls = re.findall(r'[\'"]([^\'"]*\.js)[\'"]',
                                             script.string)
                ...
                print(
                    f"{Fore.RED}Error parsing HTML from {url}: {e}{Style.RESET_ALL}"
                )
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                for depth in range(int(max_depth) + 1):
                    ...
                    tasks = [
                        bounded_get_js_links(session, url, domain)
                        for url in to_visit
                    ]
                    ...
                    for result in results:
                        ...
                        to_visit.update(
                            ...
                            if is_valid_url(link) and is_same_domain(
                                link, domain) and link not in visited)
            ...
            print(
                f"{Fore.RED}Unexpected error during crawl: {e}{Style.RESET_ALL}"
            )
            return set()
    ...
    async def main():
        try:
            ...
            js_files = await crawl_website(args.j, args.depth,
                                           args.concurrency)
            ...
            print(
                f"{Fore.RED}Unexpected error in main function: {e}{Style.RESET_ALL}"
            )
...
if args.smuggler:
    ...
    commands(
        f"python3 {smug_path}/tools/smuggler/smuggler.py -u {args.smuggler} -q"
    )
...
if args.api_fuzzer:
    error_patterns = [
        "404", "Page Not Found", "Not Found", "Error 404", "404 Not Found",
        "The page you requested was not found",
        "The requested URL was not found", "This page does not exist",
        "The requested page could not be found",
        "Sorry, we couldn't find that page", "Page doesn't exist"
    ]
    s = requests.Session()
    ...
    def check_endpoint(endpoint):
        ...
        try:
            ...
            if soup.find("title") and "404" in soup.find("title").text.lower():
                pass
            elif soup.find("title") and "Page Not Found" in soup.find(
                    "title").text.lower():
                pass
            ...
            for header in headers:
                ...
                if r.status_code == 200:
                    print(Fore.WHITE + url + ' ' +
                          json.dumps(list(header.items())[-1]) + Fore.GREEN +
                          " [{}]".format(r.status_code))
                else:
                    print(Fore.WHITE + url + ' ' +
                          json.dumps(list(header.items())[-1]) + Fore.RED +
                          " [{}]".format(r.status_code))
    ...
    if args.wordlist:
        if args.threads:
            ...
            def filter_wordlist(wordlist, extensions):
                ...
                return [
                    word for word in wordlist if any(
                        word.endswith(ext) for ext in ext_list)
                ]

            def dorequests(wordlist: str, base_url: str, headers: dict,
                           is_file_only: bool, excluded_codes: set, bar,
                           print_lock):
                s = requests.Session()
            ...
            def main():
                ...
                excluded_codes = set(
                    int(code.strip()) for code in args.exclude.split(',')
                    if code.strip())
                ...
                with alive_bar(len(filtered_wordlist),
                               title="Scanning",
                               bar="classic",
                               spinner="classic") as bar:
                    with ThreadPoolExecutor(
                            max_workers=int(args.threads)) as executor:
                        futures = [
                            executor.submit(dorequests, wordlist,
                                            args.directorybrute, headers,
                                            is_file_only, excluded_codes, bar,
                                            print_lock)
...
if args.print_all_ips:
    ...
    def extract_ips(subnet, max_workers=100):
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = [
                executor.submit(extract_ip, ip) for ip in network.hosts()
            ]
            for future in as_completed(futures):
...
if args.xss_scan:
    ...
    def encode_payload(payload):
        encodings = [
            ...
            lambda x: ''.join(f'%{ord(c):02X}'
                              for c in x),  # Full URL encoding
            lambda x: ''.join(f'&#x{ord(c):02X};'
                              for c in x),  # Hex entity encoding
        ...
        for param in params:
            for payload in payloads:
                ...
                test_url = parsed_url._replace(
                    query=urlencode(test_params, doseq=True)).geturl()

                try:
                    response = rate_limited_request(test_url,
                                                    headers=header,
                                                    timeout=10)
                    ...
                    if random_string.lower() in response_text:
                        ...
                        pattern_script = r'<script>.*?alert\([\'"]{}[\'"]\).*?</script>'.format(
                    ...
                    bar(
                    )  # Increment the progress bar for each payload scanned
    ...
    def xss_scanner(target):
        if os.path.isfile(target):
            ...
            all_vulnerabilities = []
            with alive_bar(total_payloads,
                           title="Scanning XSS Vulnerabilities") as bar:
                with ThreadPoolExecutor(
...
if args.sqli_scan:
    ...
    def sqli_scan_url(url, print_queue, bar, rate_limiter):
        ...
        for param in params:
            ...
            for payload in error_payloads:
                ...
                try:
                    ...
                    sql_errors = [
                        ...
                        r"SQL Server.*Driver", r"Warning.*mssql_.*",
                        r"Microsoft SQL Native Client error '[0-9a-fA-F]{8}",
                        r"ODBC SQL Server Driver", r"SQLServer JDBC Driver",
                        r"Oracle error", r"Oracle.*Driver",
                    ...
                    for error in sql_errors:
                        if re.search(error, response.text, re.IGNORECASE):
                            ...
                            bar(
                            )  # Increment progress bar upon finding a vulnerability
            ...
            true_url = parsed_url._replace(
                query=urlencode(original_params, doseq=True)).geturl()
            ...
            try:
                ...
                bar(
                )  # Increment the progress bar even if vulnerability is found
            ...
            time_url = parsed_url._replace(
                query=urlencode(time_params, doseq=True)).geturl()
    ...
    def sqli_scanner(target):
        ...
        try:
            ...
            print(
                f"{Fore.RED}Payload file 'payloads/sqli.txt' not found.{Fore.RESET}"
            )
            return []
        ...
        rate_thread = threading.Thread(target=release_rate_limiter,
                                       daemon=True)
        rate_thread.start()

        with alive_bar(total_payloads,
                       title="Scanning SQL Injection Vulnerabilities") as bar:
            with ThreadPoolExecutor(max_workers=args.concurrency) as executor:
                future_to_url = {
                    executor.submit(sqli_scan_url, url, print_queue, bar, rate_limiter):
                    url
                    for url in urls
                }
                for future in as_completed(future_to_url):
...
if args.webserver_scan:
    ...
    def detect_web_server(url):
        ...
        if status is None:
            print(
                f"{Fore.RED}Error: Unable to connect to the server{Style.RESET_ALL}"
            )
            return
        ...
        if 'Server' not in server_info:
            if 'Set-Cookie' in headers and 'ASPSESSIONID' in headers[
                    'Set-Cookie']:
                server_info['Likely'] = 'IIS'
            elif 'Set-Cookie' in headers and 'PHPSESSID' in headers[
                    'Set-Cookie']:
                ...
                if findings:
                    ...
                    for name, matches in findings.items():
                        ...
                        for match in matches[:
                                             5]:  # Limit to first 5 matches to avoid overwhelming output
                            print(f"    {match}")
                        if len(matches) > 5:
...
if args.javascript_endpoints:
    ...
    async def process_js_files(file_path, concurrency):
        ...
        try:
            ...
            print(
                f"{Fore.RED}Error processing JS file list: {e}{Style.RESET_ALL}"
            )
    ...
    async def main():
        ...
        js_files = await process_js_files(args.javascript_endpoints,
                                          args.concurrency)
...
if args.param_miner:
    ...
    def analyze_response_difference(original_response, modified_response):
        if abs(
                len(original_response.content) -
                len(modified_response.content)) > 50:
            return True
        return False
    ...
    def scan_common_parameters(url):
        common_params = [
            'id', 'page', 'search', 'q', 'query', 'file', 'filename', 'path',
            'dir'
        ]
        found_params = []
        for param in common_params:
            result, _ = brute_force_parameter(url, param,
    ...
    def main(url, wordlist, threads):
        print(
            f"{Fore.BLUE}[*] Starting parameter mining on: {url}{Style.RESET_ALL}"
        )

        original_response = requests.get(url, timeout=5)

        print(
            f"{Fore.MAGENTA}[*] Scanning for common parameters...{Style.RESET_ALL}"
        )
        ...
        all_params = list(
            set(wordlist_params + extracted_params + common_params))
        ...
        with ThreadPoolExecutor(max_workers=threads) as executor:
            futures = [
                executor.submit(brute_force_parameter, url, param,
                                original_response) for param in all_params
            ]
            for future in as_completed(futures):
...
if args.custom_headers:
    ...
    def main(initial_url, verbose):
        ...
        while True:
            ...
            choice = input(
                f"{Fore.CYAN}Enter your choice (1-11): {Style.RESET_ALL}")
            ...
            if choice == '1':
                ...
                method = input(
                    f"{Fore.CYAN}Enter HTTP method: {Style.RESET_ALL}").upper(
                    )
                custom_headers = {}
                print(
                    f"{Fore.YELLOW}Enter custom headers (one per line, format 'Key: Value'). Type 'done' when finished.{Style.RESET_ALL}"
                ...
                url = input(
                    f"{Fore.CYAN}Enter the new URL to check: {Style.RESET_ALL}"
                )
            elif choice == '6':
                filename = input(
                ...
                username = input(
                    f"{Fore.CYAN}Enter username: {Style.RESET_ALL}")
                password = input(
                    f"{Fore.CYAN}Enter password: {Style.RESET_ALL}")
                ...
                if response:
                    ...
                    print(
                        f"{Fore.GREEN}Response saved to {filename}{Style.RESET_ALL}"
                    )
            elif choice == '11':
...
if args.openredirect:
                ...
                if location:
                    ...
                    if parsed_location.netloc and parsed_location.netloc != original_netloc:
                        ...
                        if TEST_DOMAIN in parsed_location.netloc:
                            print(
                                f"{RED}VULNERABLE: Redirects to {location}{RESET}"
                            )
    ...
    def test_open_redirect(url):
        ...
        with concurrent.futures.ThreadPoolExecutor(
                max_workers=args.concurrency) as executor:
            future_to_payload = {
                executor.submit(test_single_payload, url, payload, original_netloc):
                payload
                for payload in PAYLOADS
            }
...
if args.haveibeenpwned:
    ...
    print(
        f"Checking password: {Fore.GREEN}{args.haveibeenpwned}{Style.RESET_ALL}\n"
    )
...
if args.subdomaintakeover:
    COMMON_SERVICES = [
        "GitHub, Inc.", "GitLab Inc.", "Bitbucket", "Heroku, Inc.", "Firebase",
        "Netlify, Inc.", "Surge", "Automattic Inc.", "Amazon CloudFront",
        "Microsoft Azure", "Google LLC"
    ]
...
if args.autorecon:
    ...
    async def ssl_vuln_scan(target):
        ...
        def check_ssl(domain: str, port: int = 443):
            try:
                ...
                with socket.create_connection((domain, port),
                                              timeout=5) as sock:
            ...
            if response_text:
                ...
                for link in soup.find_all('a', href=True):
                    ...
                    if not full_url.startswith('http'):
                        full_url = urlparse(target)._replace(
                            path=full_url).geturl()
    ...
    def extract_parameters(links):
        print(
            f"{Fore.MAGENTA}Extracting parameters from links...{Style.RESET_ALL}"
        )
        parameters = {}
        for link in links:
    ...
    def shodan_search(target, api):
        ...
        try:
            ...
            for match in results['matches']:
                ip = match['ip_str']
                port = match['port']
                services = match.get('product',
                                     'Unknown')  # Get the service/product name
                extracted_data.append(
                    f"IP: {ip}, Port: {port}, Service: {services}")
    ...
    async def main(target):
        ...
        shodankey = input(
            f"{Fore.CYAN}Enter your Shodan API key: {Style.RESET_ALL}")
        print("\n")
        with alive_bar(12, title='Running autorecon') as bar:
            ...
            print(
                f"{Fore.MAGENTA}Server: {Fore.CYAN}{serverinfo}{Style.RESET_ALL}"
            )
            bar()
            ...
            print(
                f"{Fore.MAGENTA}Tech Detect: {Fore.CYAN}{tech}{Style.RESET_ALL}"
            )
